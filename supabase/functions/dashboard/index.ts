  ...
  try {
    ...
    if (path === "/overview" || path === "/overview/") {
      if (req.method === "GET") {
        ...
        const { data: brandProfiles } = await supabase.from("brand_profiles").select("*").eq("client_id", clientId).order("created_at", {
          ascending: false
        });
        ...
        const { data: discoveredCompanies } = await supabase.from("discovered_companies").select("id, enrichment_status").in("brand_id", brandIds.length > 0 ? brandIds : emptyBrandList);
        const { data: discoveredContacts } = await supabase.from("discovered_contacts").select("id").in("brand_id", brandIds.length > 0 ? brandIds : emptyBrandList);
        const companiesTotal = discoveredCompanies?.length || 0;
        const companiesPending = discoveredCompanies?.filter((c)=>c.enrichment_status === "pending").length || 0;
        ...
        const { data: systemFlags } = await supabase.from("system_flags").select("*").eq("client_id", clientId).maybeSingle();
        // Activity feed - recent 20 activities
        const { data: activities } = await supabase.from("activity_logs").select("*").eq("client_id", clientId).order("created_at", {
          ascending: false
        }).limit(20);
        // Get enrichment pending count
        const { data: enrichmentPending } = await supabase.from("discovered_companies").select("id", {
          count: "exact"
        }).in("brand_id", brandIds.length > 0 ? brandIds : emptyBrandList).eq("enrichment_status", "pending");
        return new Response(JSON.stringify({
    ...
    if (brandIdMatch && (req.method === "PATCH" || req.method === "PUT")) {
      ...
      const { data: brand } = await supabase.from("brand_profiles").select("*").eq("id", brandId).eq("client_id", clientId).single();
      if (!brand) {
        return new Response(JSON.stringify({
      ...
      if (body.discovery_enabled !== undefined || body.outbound_enabled !== undefined) {
        ...
        if (!memberRole || ![
          ...
          return new Response(JSON.stringify({
            error: "Only admins can toggle workers"
          }), {
            status: 403,
            headers: {
              ...corsHeaders,
      ...
      const { data: updatedBrand, error } = await supabase.from("brand_profiles").update(updates).eq("id", brandId).select().single();
      if (error) {
        return new Response(JSON.stringify({
          error: error.message
        }), {
