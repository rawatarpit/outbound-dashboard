...
import { Sparkles, Trash2, BarChart2, MessageSquare, Clock, Sparkles as SparklesIcon } from 'lucide-react'
...
export default function ChatSidebar() {
  ...
  const { sessionId, messages, isStreaming, processing, error, clearChat } = useChat()
  ...
  const sessionTitle = firstUserMessage ? firstUserMessage.slice(0, 50) + (firstUserMessage.length > 50 ? '…' : '') : 'New chat'
    ...
    <aside className="hidden lg:flex w-[280px] shrink-0 flex-col border-r border-border bg-card overflow-y-auto">
      ...
      <div className="px-5 py-5 border-b border-border">
        ...
        <div className="space-y-2 mb-3">
          <p className="text-sm font-medium text-foreground truncate">{sessionTitle}</p>
          <div className="grid grid-cols-3 gap-2 text-center">
            ...
            <div className="bg-muted/50 rounded-lg px-2 py-1.5">
              ...
              <p className="text-lg font-semibold text-primary">{userMessages}</p>
            </div>
            <div className="bg-muted/50 rounded-lg px-2 py-1.5">
              <p className="text-xs text-muted-foreground">AI</p>
              <p className="text-lg font-semibold text-emerald-600">{assistantMessages}</p>
            </div>
          </div>
        </div>
            ...
            {processing && (
              <div className="space-y-1.5">
                ...
                <p className="text-xs text-muted-foreground">
                  Step {processing.currentStep} of {processing.steps.length}
                </p>
              </div>
            )}
          </div>
        ...
        {error && (
          <div className="mb-3 p-3 rounded-lg bg-destructive/10 border border-destructive/20">
            ...
            <p className="text-xs text-destructive/80 mt-0.5 truncate">{error}</p>
          </div>
        )}
      </div>
      ...
      <div className="px-5 py-5 mt-auto border-t border-border">
        <div className="space-y-1.5">
          <button
            onClick={clearChat}
            className="w-full inline-flex items-center justify-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium text-destructive hover:bg-destructive/10 transition-colors"
          >
            <Trash2 className="h-3 w-3" />
            Clear Chat
          </button>
          {sessionId && totalMessages > 0 && (
            <a
              href={`/chat?session=${sessionId}`}
              className="w-full inline-flex items-center justify-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium text-primary hover:bg-primary/10 transition-colors border border-primary/20"
            >
