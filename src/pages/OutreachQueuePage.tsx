...
import { Mail, Send, XCircle, Eye, Building2, Edit3, Save, Clock, AlertCircle, CheckCircle, TrendingUp, Users, Activity, Loader2 } from 'lucide-react'
...
export default function OutreachQueuePage() {
  ...
  const awaitingApproval = entries.filter(e => e.status === 'draft').length
  const processingCount = entries.filter(e => e.status === 'draft_processing').length
  const totalCompanies = new Set(entries.map(e => e.company_id)).size
  ...
  return (
    <div className="space-y-6">
        ...
        <div className={`relative overflow-hidden rounded-xl border p-5 ${
          ...
          <div className="relative flex items-center gap-4">
            <div className={`h-10 w-10 rounded-xl flex items-center justify-center ${
              processingCount > 0 ? 'bg-amber-500/15' : 'bg-green-500/15'
            }`}>
              {processingCount > 0
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold text-foreground">
                {processingCount > 0 ? `${processingCount} draft(s) processing` : 'All drafts ready for review'}
              </p>
              <p className="text-xs text-muted-foreground mt-0.5">
                {processingCount > 0
                  ? 'Some drafts are still being generated by the engine'
                  : `${awaitingApproval} draft(s) awaiting approval`
                }
              </p>
            </div>
            <div className="hidden sm:flex items-center gap-4 text-xs text-muted-foreground">
              <span className="flex items-center gap-1.5"><Mail className="h-3.5 w-3.5" />{pendingCount} total</span>
              <span className="flex items-center gap-1.5"><Users className="h-3.5 w-3.5" />{totalCompanies} companies</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="grid gap-4 grid-cols-2 md:grid-cols-4">
        <StatCard icon={Mail} label="Pending Drafts" value={<AnimatedCounter value={pendingCount} />}
          subvalue={processingCount > 0 ? `${processingCount} processing` : undefined} color="#3b82f6" />
        <StatCard icon={Clock} label="Created Today" value={<AnimatedCounter value={todayCount} />}
          subvalue={`${((todayCount / Math.max(pendingCount, 1)) * 100).toFixed(0)}% of total`} color="#8b5cf6" />
        <StatCard icon={CheckCircle} label="Awaiting Approval" value={<AnimatedCounter value={awaitingApproval} />}
          subvalue={processingCount > 0 ? `${processingCount} in progress` : 'ready to send'} color="#22c55e" />
        <StatCard icon={Building2} label="Total Companies" value={<AnimatedCounter value={totalCompanies} />}
          subvalue={`${pendingCount} drafts generated`} color="#f59e0b" />
      </div>

      {entries.length === 0 ? (
        <Card className="rounded-2xl border-border/50 bg-card/50 backdrop-blur-sm">
          <CardContent className="flex flex-col items-center justify-center py-16">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-5" style={{ backgroundColor: '#3b82f612' }}>
        ...
        <div className="space-y-4">
          <SectionHeader icon={Mail} title="Outreach Drafts" subtitle={`${pendingCount} draft(s) pending review`} />
          {entries.map((entry) => {
                ...
                <CardContent className="p-5">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="h-11 w-11 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: '#6366f112' }}>
                    ...
                    <Badge className={`shrink-0 ml-3 ${isProcessing ? 'bg-amber-500/15 text-amber-600 border-amber-500/30' : 'bg-blue-500/15 text-blue-600 border-blue-500/30'}`}>
          ...
          {pendingCount > 0 && (
            <Card className="rounded-2xl border-border/50 bg-card/50 backdrop-blur-sm">
              <CardContent className="p-5">
                <SectionHeader icon={TrendingUp} title="Draft Pipeline" subtitle="Draft → sent conversion" />
                <div className="grid grid-cols-3 gap-4">
                  <div className="text-center p-4 rounded-xl bg-muted/30 border border-border/50">
                    <p className="text-2xl font-bold text-foreground"><AnimatedCounter value={pendingCount} /></p>
                    <p className="text-xs text-muted-foreground mt-1">Total Drafts</p>
                  </div>
                  <div className="text-center p-4 rounded-xl bg-muted/30 border border-border/50">
                    <p className="text-2xl font-bold text-foreground"><AnimatedCounter value={awaitingApproval} /></p>
                    <p className="text-xs text-muted-foreground mt-1">Awaiting Approval</p>
                  </div>
                  <div className="text-center p-4 rounded-xl bg-green-500/5 border border-green-500/20">
                    <p className="text-2xl font-bold text-green-600">
                      {pendingCount > 0 ? `${Math.round((awaitingApproval / pendingCount) * 100)}%` : '0%'}
                    </p>
